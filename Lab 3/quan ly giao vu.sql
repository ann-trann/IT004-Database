...
/* III. Ngoon ngu truy van du lieu */

-- 1.
SELECT MAHV, CONCAT(ho, ' ', ten) HOTEN, NGSINH, HV.MALOP
FROM HOCVIEN HV
		JOIN LOP ON HV.MAHV = LOP.TRGLOP

-- 2.
SELECT HV.MAHV, CONCAT(HO, ' ', TEN) HOTEN, LANTHI, DIEM
FROM HOCVIEN HV
		JOIN KETQUATHI KQT ON HV.MAHV = KQT.MAHV
		JOIN MONHOC MH ON MH.MAMH = KQT.MAMH
WHERE MH.MAMH = 'CTRR' AND MALOP = 'K12'
ORDER BY TEN ASC, HO DESC

-- 3.
SELECT HV.MAHV, CONCAT(HO, ' ', TEN) HOTEN, TENMH
FROM HOCVIEN HV
		JOIN KETQUATHI KQT ON HV.MAHV = KQT.MAHV
		JOIN MONHOC MH ON MH.MAMH = KQT.MAMH
